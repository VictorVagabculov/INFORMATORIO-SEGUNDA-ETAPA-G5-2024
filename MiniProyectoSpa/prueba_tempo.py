...
# Temporizador
tiempo_total = 0
label_tiempo = tk.Label(ventana, text="00:00:00", font=("Arial", 45), bg="lightblue")
label_tiempo.grid(row=2, column=4, columnspan=1, padx=200, pady=100)
...
boton_iniciar = tk.Button(ventana, text="▶", command=iniciar_temporizador, font=("Arial", 14, "bold"), bg="lightblue", borderwidth=3)
boton_iniciar.grid(row=2, column=2, columnspan=2, padx=0, pady=100)

# Función para agregar tiempo al temporizador
def agregar_tiempo(valor):
    global tiempo_total
    tiempo_total += valor
    horas, minutos = divmod(tiempo_total, 60)
    segundos = minutos % 60
    label_tiempo.config(text=f"{horas:02}:{minutos:02}:{segundos:02}")
